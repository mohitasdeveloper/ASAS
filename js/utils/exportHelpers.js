...
const COLLEGE_LOGO_URL  = 'https://i.ibb.co/zVfPWWBJ/bknc.png';
const COLLEGE_NAME      = 'B. K. Birla Night College, Kalyan';
const COLLEGE_SUB1      = '(Affiliated to University of Mumbai)';
const COLLEGE_SUB2      = 'Commerce & Management';
...
export function exportToPDF(rows, columns, title, filename, subtitle = '', generatedBy = '') {
  ...
  doc.autoTable({
    ...
    columnStyles: Object.fromEntries(columns.map((c, i) => [i, { cellWidth: c.pdfWidth ?? 'auto' }])),
      ...
      doc.text(`Report Generated By: ${generatedBy || 'ASAS'}`, 14, pageH - 9);
...
function _drawHeader(doc, pageW, reportTitle, subtitle) {
  ...
  // Logo (left)
  if (window._ASAS_LOGO_DATA) {
    try { doc.addImage(window._ASAS_LOGO_DATA, 'PNG', 14, 8, 28, 28); } catch(e) {}
  }
 ...
 * Fetches the logo and caches as base64 in window._ASAS_LOGO_DATA.
 *
 *   import { preloadLogoForPDF } from '/js/utils/exportHelpers.js';
 *   preloadLogoForPDF();
 */
export function preloadLogoForPDF() {
  if (window._ASAS_LOGO_DATA) return;
  const img = new Image();
  ...
  img.onload = () => {
    try {
      ...
      window._ASAS_LOGO_DATA = canvas.toDataURL('image/png');
    } catch(e) { console.warn('[ASAS] Logo preload failed:', e.message); }
  };
